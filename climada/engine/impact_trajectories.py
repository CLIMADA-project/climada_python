...
class TBRSnapshots():
    def __init__(self, exposure_set, hazard_set, impfset, TBR_snapyears):
        self.exposure_set = exposure_set
        self.hazard_set = hazard_set
        self.impfset = impfset
        self.TBR_snapyears = TBR_snapyears
        self.data_dict = {
            year: [exposure_set[year], hazard_set[year]] for year in TBR_snapyears
        }
    ...
    def from_dict(cls, snapshots_dict, impfset):
        TBR_snapyears = list(snapshots_dict.keys())
        exposure_set = {year:snapshots_dict[year][0] for year in TBR_snapyears}
        hazard_set = {year:snapshots_dict[year][1] for year in TBR_snapyears}
        return cls(exposure_set=exposure_set, hazard_set=hazard_set, impfset=impfset,TBR_snapyears=TBR_snapyears)

    @classmethod
    def from_lists(cls, hazard_list, exposure_list, impfset, TBR_snapyears):
        exposure_set = {year:exposure_list[i] for i,year in enumerate(TBR_snapyears)}
        hazard_set = {year:hazard_list[i] for i,year in enumerate(TBR_snapyears)}
        return cls(exposure_set=exposure_set, hazard_set=hazard_set, impfset=impfset,TBR_snapyears=TBR_snapyears)

class CalcImpactsSnapshots():
    def __init__(self, snapshots:TBRSnapshots):
    ...
    def interpolate_eai_exp(self, imp0,imp1,start_year,end_year,frequency):
        ...
        for year in range(start_year, end_year + 1):
            imp_mat_intrpl = interpolate_sm(imp0.imp_mat, imp1.imp_mat, year, start_year, end_year)
    ...
    def snapshot_combinaisons(self, year0, year1):
        ...
        imp_E1H0  = ImpactCalc(exp_y1, self.snapshots.impfset, haz_y0).impact()# Impact 2)  Hazard 2000  and Exposure 2020
    ...
    def bayesian_mixer(self):
        ...
        for i in range(len(self.snapshots.TBR_snapyears)-1):
            start_year,end_year = self.snapshots.TBR_snapyears[i], self.snapshots.TBR_snapyears[i+1]
            prop_H0, prop_H1, imp_E0H0, imp_E1H0, imp_E0H1, imp_E1H1 = self.snapshot_combinaisons(start_year,end_year)
            frequency0 = self.snapshots.data_dict[start_year][1].frequency
            frequency1 = self.snapshots.data_dict[end_year][1].frequency

            yearly_eai_exp_0 = self.interpolate_eai_exp(imp_E0H0, imp_E1H0,start_year,end_year,frequency0)
            yearly_eai_exp_1 = self.interpolate_eai_exp(imp_E0H1, imp_E1H1,start_year,end_year,frequency1)
            yearly_aai_0 = [ImpactCalc.aai_agg_from_eai_exp(eai_exp) for eai_exp in yearly_eai_exp_0]
            yearly_aai_1 = [ImpactCalc.aai_agg_from_eai_exp(eai_exp) for eai_exp in yearly_eai_exp_1]
...
class TBRTrajectories():
    ...
    def create_hazard_yearly_set(cls, haz: Hazard):
        ...
        for year in range(years.min(),years.max(),1):
            haz_set[year] = haz.select(date=[f"{str(year)}-01-01",f"{str(year+1)}-01-01"])
    ...
    def create_exposure_set(cls, snapshot_years, exp1, exp2=None, growth=None):
        ...
        if exp2 is None:
            if growth is None:
                ...
                exp_set = {year:grow_exp(exp1, year-year_0) for year in snapshot_years}
        else:
            exp_set = {year:interp(exp1, exp2, year-year_0) for year in snapshot_years}
