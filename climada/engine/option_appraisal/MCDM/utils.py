    ...
    if filter_conditions:
        for col, cond in filter_conditions.items():
            ...
            if isinstance(cond, list):
                ...
                boolean_df[col] = (unfiltered_df[col] >= lower) & (
                    unfiltered_df[col] <= upper
                )
...
def generate_metrics(
    ...
    if unc_func_dist_dict:
        for unc_var in unc_func_dist_dict.keys():
            if (
                unc_func_dist_dict[unc_var]["func"]
                and unc_func_dist_dict[unc_var]["distr"]
            ):
    ...
    for ent_key, haz_key, aggr_set in itertools.product(
        ent_dict.keys(), haz_dict.keys(), aggr_sets
    ):
        # Clear the output
        clear_output()
        ...
        if aggr_set:
            aggr_combos_df = (
                ent_dict[ent_key]["today"].exposures.gdf[aggr_set].drop_duplicates()
            )
        else:
        ...
        for index, aggr_combo in aggr_combos_df.iterrows():
            ...
            if aggr_set:
                ...
                # Enitiy today
                idx_combo = (
                    ent_today.exposures.gdf[aggr_set]
                    .isin(aggr_combo.values)
                    .all(axis=1)
                )
                ent_today.exposures.gdf = ent_today.exposures.gdf[idx_combo]
                # Entity future
                idx_combo = (
                    ent_fut.exposures.gdf[aggr_set].isin(aggr_combo.values).all(axis=1)
                )
            ...
            haz_today = haz_dict[haz_key]["today"].select(
                extent=(
                    ent_today.exposures.gdf.longitude.min(),
                    ent_today.exposures.gdf.longitude.max(),
                    ent_today.exposures.gdf.latitude.min(),
                    ent_today.exposures.gdf.latitude.max(),
                )
            )
            haz_fut = haz_dict[haz_key]["future"].select(
                extent=(
                    ent_fut.exposures.gdf.longitude.min(),
                    ent_fut.exposures.gdf.longitude.max(),
                    ent_fut.exposures.gdf.latitude.min(),
                    ent_fut.exposures.gdf.latitude.max(),
                )
            )
            ...
            if not unc_func_dist_dict:
                ...
                if aggr_set:
                    t_dict_to_df.update(
                        {
                            col_name: [col_value for meas in measure_names]
                ...
                t_dict_to_df["crit_cost"] = [
                    measure_dict[meas].cost if meas != "no measure" else 0
                ...
                for name, risk_fcn in risk_fncs_dict.items():
                    ...
                    t_dict_to_df["crit_ben_" + name] = [
                        costbenefit_disc.benefit[meas] if meas != "no measure" else 0
                        for meas in measure_names
                    ]
                    t_dict_to_df["crit_bcr_" + name] = [
                        (
                            1 / costbenefit_disc.cost_ben_ratio[meas]
                            if meas != "no measure"
                            else 0
                        )
                        for meas in measure_names
                    ...
                    t_dict_to_df["crit_npv_" + name] = [
                        (
                            costbenefit_disc.tot_climate_risk
                            - costbenefit_disc.benefit[meas]
                            if meas != "no measure"
                ...
                metrics_df = pd.concat(
                    [metrics_df, pd.DataFrame(t_dict_to_df)], ignore_index=True
                )
                ...
                for key in keys:
                    if (
                        unc_func_dist_dict[key]["func"]
                        and unc_func_dist_dict[key]["distr"]
                    ):
                        iv_dict[key + "_iv"] = InputVar(
                            partial(unc_func_dist_dict[key]["func"], **base_dict),
                            unc_func_dist_dict[key]["distr"],
                        )
                    else:
                ...
                if first_iteration:
                    ...
                    df_samples = pd.concat(
                        [df_samples, pd.DataFrame(new_mean_row)], ignore_index=True
                    )
                ...
                for name, risk_fcn in risk_fncs_dict.items():
                    ...
                    for meas in measure_names:
                        if meas == "no measure":
                            ...
                            t_meas_crit_df["crit_npv_" + name] = df_results[
                                "tot_climate_risk"
                            ]
                            t_crit_df = pd.concat(
                                [t_crit_df, t_meas_crit_df], ignore_index=True
                            )
                        else:
                            t_meas_crit_df["measure"] = meas
                            t_meas_crit_df["crit_ben_" + name] = df_results[
                                meas + " Benef"
                            ]
                            t_meas_crit_df["crit_bcr_" + name] = (
                                1 / df_results[meas + " CostBen"]
                            )
                            t_meas_crit_df["crit_npv_" + name] = (
                                df_results["tot_climate_risk"]
                                - df_results[meas + " Benef"]
                            )
                            t_crit_df = pd.concat(
                                [t_crit_df, t_meas_crit_df], ignore_index=True
                            )
    ...
    col = "crit_cost"
    meas_cost_df = (
        metrics_df[["measure", col]]
        .dropna()
        .drop_duplicates()
        .rename(columns={col: "clean"})
    )
    ...
    base_col = [
        col for col in metrics_df.columns if "crit" not in col and "entity" not in col
    ]
    piv_col = ["entity"]
    ...
    metrics_df.columns = [
        f'{"_".join(col)}' if col[1] else f"{col[0]}" for col in metrics_df.columns
    ]
    ...
    metrics_df.columns = [
        col.replace("crit_", "") if "crit" in col else col for col in metrics_df.columns
    ]
...
def generate_unc_func_dist_dict(func_dict, unc_var_dist_dict):
    ...
    for func_name, func in func_dict.items():
        ...
        distr = {
            arg: unc_var_dist_dict[arg] for arg in arg_names if arg in unc_var_dist_dict
        }
