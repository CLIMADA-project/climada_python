...
class TestOpenStreetMapModule(unittest.TestCase):
    ...
    def test_make_osmexposure(self):
        ...
        self.assertEqual(buildings_47_8_default.loc[random.randint(0,len(buildings_47_8_default))].geometry.type, "Point")
        self.assertGreater(buildings_47_8_default.loc[random.randint(0,len(buildings_47_8_default))].value, 0)
...
class TestOSMlongUnitTests(unittest.TestCase):
    ...
    def test_split_exposure_highlow(self):
        ...
        for mode in {'proportional','even'}:
            ...
            self.assertTrue(math.isclose(sum(exp_sub_high.value), sum(exp_sub.value), rel_tol=0.01))
