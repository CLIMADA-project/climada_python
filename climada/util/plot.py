...
from climada.util.constants import CMAP_CAT, CMAP_EXPOSURES, CMAP_RASTER
...
def plot_from_gdf(
    ...
    **kwargs,
    ...
    if not callable(title_subplots):
        print(
            "Unknown subplot-title-generation function. Subplot titles will be column names."
        )
        title_subplots = lambda cols: [f"{col}" for col in cols]

    # use log colorbar for return periods and impact
    if (
        colorbar_name.strip().startswith(("Return Period", "Impact"))
        and "norm" not in kwargs.keys()
        and
        # check if there are no zeros values in gdf
        not np.any(gdf_values == 0)
        and
    ...
    if (
        colorbar_name.strip().startswith("Return Period")
        and "cmap" not in kwargs.keys()
    ):
