...
def eis2ais(eis, hazard= None, number_of_years=None, amount_events=None, 
    ...
    # sollte ich nur die frequency der non-zero events zusammenrechnen?
    
    # INITIALISATION
    # artificially generate a generic yearset (by just sampling the event impact set)
    # annual impact set, event impact set
    ...
    #ais['ais'] = True #add a flag to distinguish ais from other impact class objects
    ...
    #das hier stimmt irgendwie noch nicht
    #adjust for sampling error (i.e. rand will not pick each impact once...)
    ais_ed = np.sum(impact_per_year)/number_of_years #hier ist der Fehler
    eis_ed = np.sum(eis.frequency*eis.at_event)
    correction_factor = eis_ed/ais_ed
    #das printen funktioiert noch nicht
    LOGGER.info("The correction factor amounts to"+str((correction_factor-1)*100)+"%")
...
def sampling_poisson(number_of_years, n_annual_events, nonzero_pos):
    amount_events_per_year = np.round(np.random.poisson(lam=n_annual_events , size=number_of_years)).astype('int')
