...
class TropCyclone(Hazard):
    ...
    def set(self, files, descriptions='', centroids=None, model='H08'):
        ...
        tc_part = Pool().map(self._set_one, all_files, desc_list, centr_list,
                             model_list)
        for tc_haz, file in zip(tc_part, all_files):
        ...
        # TODO add computation with original tracks
        self.frequency = self.frequency / delta_time
...
def read_ibtracs(file_name):
    ...
    track_ds.attrs['orig_event_flag'] = dfr['original_data'].values[0]
