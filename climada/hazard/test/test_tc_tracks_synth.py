...
class TestDecay(unittest.TestCase):
    def test_apply_decay_no_landfall_pass(self):
        ...
        self.assertTrue(
            np.allclose(
                tc_track.data[0].central_pressure.values, tc_ref.central_pressure.values
            )
        )
        self.assertTrue(
            np.allclose(
    ...
    def test_decay_end_ocean(self):
        ...
        p_hist_lf = np.concatenate(
            [track_hist.central_pressure.values[lfs:lfe] for lfs, lfe in zip(*lf_idx)]
        )
        p_synth_lf = np.concatenate(
            [track.central_pressure.values[lfs:lfe] for lfs, lfe in zip(*lf_idx)]
        )
        v_hist_lf = np.concatenate(
            [track_hist.max_sustained_wind.values[lfs:lfe] for lfs, lfe in zip(*lf_idx)]
        )
        ...
        self.assertTrue(
            np.all(track.central_pressure.values <= track.environmental_pressure.values)
        )
...
class TestSynth(unittest.TestCase):
    ...
    def test_random_walk_ref_pass(self):
        ...
        self.assertEqual(tc_track.data[1].id_no, 1.951239012334010e12)
        ...
        self.assertAlmostEqual(tc_track.data[2].id_no, 1.951239012334020e12)
