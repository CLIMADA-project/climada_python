...
class TestIbtracs(unittest.TestCase):
    ...
    def test_read_with_basin(self):
        ...
        tc_track = tc.TCTracks()
        tc_track.read_ibtracs_netcdf(year_range=(1991, 1991), basin="SP", estimate_missing=True)
        self.assertEqual(tc_track.size, 9)
        self.assertEqual(tc_track.data[0].attrs['basin'], 'SI')
        self.assertEqual(tc_track.data[1].attrs['basin'], 'SP')
        ...
        for tr in tc_track.data:
            self.assertEqual(tr.attrs['basin'], "NI")
        ...
        self.assertEqual(tc_track.size, 3)
        for tr in tc_track.data:
            self.assertEqual(tr.attrs['basin'], "EP")
