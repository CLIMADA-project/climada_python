...
class TestKnutson(unittest.TestCase):
    ...
    def test_get_knutson_scaling_calculations(self):
        ...
        for target_year in [2030, 2050, 2070, 2090]:
            ...
            np.testing.assert_array_almost_equal(
                predicted_changes.loc[target_year, "2.6"], predicted_change[0]
            )
            np.testing.assert_array_almost_equal(
                predicted_changes.loc[target_year, "4.5"], predicted_change[1]
            )
            np.testing.assert_array_almost_equal(
                predicted_changes.loc[target_year, "6.0"], predicted_change[2]
            )
            np.testing.assert_array_almost_equal(
                predicted_changes.loc[target_year, "8.5"], predicted_change[3]
            )
    ...
    def test_get_knutson_scaling_structure(self):
        ...
        np.testing.assert_equal(
            predicted_changes.columns, np.array(["2.6", "4.5", "6.0", "8.5"])
        )
