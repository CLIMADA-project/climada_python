...
class TCTracks():
    ...
    def fetch_ecmwf_bufr(self, files):
        """Stub to eventually fetch and read ECMWF TC track predictions into memory.
        
        For the time being, simply reads a list of BUFR files into a TCTracks
        instance.
        """
        
        for i, file in enumerate(files, 1):
            self._read_one_bufr_tc(file, id_no = i)

    def _read_one_bufr_tc(self, filename, id_no=None, fcast_rep=None):
        """Read a single BUFR TC track file

        Parameters:
            filename (str): Name/path as a string, _not_ as pathlib.Path
            id_no (int): Used as a numerical ID; else use np.rand.
            fcast_rep (int): Of the form 1xx000, indicating the delayed replicator 
                containing the forecast values; optional.
        """
        ...
        if fcast_rep is None:
            # current format contains only one delayed replicator; find it
            delayed_replicators = [
                d for d in 
                mquerent.query(bufr_message, '%unexpanded_descriptors') 
                if d > 100000 and d < 200000 and d % 1000 == 0
            ]
            if len(delayed_replicators) != 1:
                LOGGER.error(('More than one delayed replicator in file ' +
                              filename + '; please set fcast_rep manually.'))
            else:
                fcast_rep = str(delayed_replicators[0])

        # check that 
        data_subcat = mquerent.query(bufr_message, '%data_i18n_subcategory')
        n_subsets = mquerent.query(bufr_message, '%n_subsets')
        
        if (data_subcat != 0) or (n_subsets == 1):
            LOGGER.info('This does not appear to be an ensemble forecast BUFR file')
        elif mquerent.query(bufr_message, '%n_subsets') < 52:
            LOGGER.info(filename + ' does not appear to contain HRES forecast results')
            
        # query the bufr message
        significance_dquery = dquerent.query(bufr_message, fcast_rep + '> 008005')
        latitude_dquery     = dquerent.query(bufr_message, fcast_rep + '> 005002')
        longitude_dquery    = dquerent.query(bufr_message, fcast_rep + '> 006002')
        wind_10m_dquery     = dquerent.query(bufr_message, fcast_rep + '> 011012')
        pressure_dquery     = dquerent.query(bufr_message, fcast_rep + '> 010051')
        timestamp_dquery    = dquerent.query(bufr_message, fcast_rep + '> 004024')
        wmo_longname_dquery = dquerent.query(bufr_message, '/001027')
        storm_id_dquery     = dquerent.query(bufr_message, '/001025')


        timestamp_origin = dt.datetime(
            mquerent.query(bufr_message, '%year'),
            mquerent.query(bufr_message, '%month'),
            mquerent.query(bufr_message, '%day'),
            mquerent.query(bufr_message, '%hour'),
            mquerent.query(bufr_message, '%minute'),
        )
        ...
        orig_centre = mquerent.query(bufr_message, '%originating_centre')
        ...
        for i in significance_dquery.subset_indices():
            ...
            ts_int = np.array(timestamp_dquery.get_values(i)).squeeze()
            ts = timestamp_origin + ts_int.astype('timedelta64[h]')
            ...
            ds.attrs['basin']: sid[2] # according to specs always num-num-letter
            saffir_scale = np.array([18, 33, 43, 50, 59, 71, 1000]) # in m/s
            ds.attrs['category'] = set_category(ds.max_sustained_wind.values,
                                                ds.max_sustained_wind_unit,
                                                saffir_scale)
