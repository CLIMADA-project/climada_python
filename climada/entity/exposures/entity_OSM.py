    ...
    if Low_Value_gdf == None:
        try:
            ...
            print('No Low-Value-Union found with name %s. \n Please add.' \
                  % (save_path +'/OSM_features_gdf_combined_'+str(int(bbox[0]))+'_'+str(int(bbox[1]))+'.shp'))
    else: 
...
def fill_highValueArea_LitPop(bbox, country, mode,  highValueArea=None, \
    ...
    if highValueArea==None:
        try:
            ...
            print('No file found of form %s. Please add or specify path.' \
                  %(os.getcwd() + 'High_Value_Area_'+str(int(bbox[0]))+'_'+str(int(bbox[1]))+".shp"))
    else:
    ...
    if mode == "nearest":       
        ...
        for i in range(0,len(ent_sub_low)):
            nearest = ent_sub_high.geometry == nearest_points(ent_sub_low.iloc[i].geometry, pointsToAssign)[1] #point
