...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr 26 11:29:44 2019
Function to download "zero value" or "low value" regions from OSM
@author: evelynm
"""
import overpy
import fiona
...
def get_features_OSM(bbox, types, save_path = os.getcwd(), check_plot = 1):
    """
     Input:
         ...
         types: List of low-value items that should be downloaded from OSM, e.g.
                recommended:
                {'natural','waterway','water','landuse' (recommended) OR 'landuse=forest', 'landuse=farmland',
                'landuse=grass', ..., 'wetland'}
         save_path: String with absolute path for saving output. Default is wdorking directory.
         check_plot = default is 1 (yes), else 0.

     Output:
         - GeoDataframes, shapefiles and plots with correct geometry (LineStrings, Polygons, MultiPolygons) 
         for each of requested low-value items saved as "item_gdf_all_lat_lon"
         - 1 combined GeoDataframe with all low-value items saved as "OSM_features_gdf_combined_lat_lon".
    """    
    for item in types:
        ###################
        ...
        try:
            ...
            print(' WARNING: !!!! \n Too many requests error - try again in a few moments \n !!!!')
        ...
        def format_shape_OSM(bbox, result_NodesFromWays, result_NodesWaysFromRels): 
            ...
            schema_poly = {'geometry': 'Polygon', 'properties': {'Name':'str:80', 'Natural_Type':'str:80', 'Item':'str:80'}}
            schema_line = {'geometry': 'LineString', 'properties': {'Name':'str:80', 'Natural_Type':'str:80','Item':'str:80'}}
            shapeout_poly = save_path + '/' + str(item)+'_poly_'+str(int(bbox[0]))+'_'+str(int(bbox[1]))+".shp"
            shapeout_line = save_path + '/' + str(item)+'_line_'+str(int(bbox[0]))+'_'+str(int(bbox[1]))+".shp"
            ...
            with fiona.open(shapeout_poly, 'w',crs=from_epsg(4326),driver='ESRI Shapefile', schema=schema_poly) as output:
                for way in way_poly:
                    geom = mapping(geometry.Polygon([node.lon, node.lat] for node in way.nodes))
                    prop = {'Name': way.tags.get("name", "n/a"),'Natural_Type': way.tags.get("natural", "n/a"),'Item': item}
                    output.write({'geometry': geom, 'properties': prop})
            
            with fiona.open(shapeout_line, 'w',crs=from_epsg(4326),driver='ESRI Shapefile', schema=schema_line) as output2:
                for way in way_line:
                    geom2 = {'type': 'LineString', 'coordinates':[(node.lon, node.lat) for node in way.nodes]}
                    prop2 = {'Name': way.tags.get("name", "n/a"),'Natural_Type': way.tags.get("natural", "n/a"),'Item': item}
                    ...
                    if relationway.role == 'outer':
                        for way in result_NodesWaysFromRels.ways:                              
                            if (way.id == relationway.ref):
                                OuterList.append(geometry.LineString([node.lon, node.lat] for node in way.nodes))
                    else:
                        for way in result_NodesWaysFromRels.ways:                              
                            if (way.id == relationway.ref):
                                InnerList.append(geometry.LineString([node.lon, node.lat] for node in way.nodes))
            ...
            shapeout_multi = save_path + '/' + str(item)+'_multi_'+str(int(bbox[0]))+'_'+str(int(bbox[1]))+".shp"
            
            with fiona.open(shapeout_multi, 'w',crs=from_epsg(4326),driver='ESRI Shapefile', schema=schema_multi) as output:
        ...
        globals()[str(item)+'_gdf_all_'+str(int(bbox[0]))+'_'+str(int(bbox[1]))] = format_shape_OSM(bbox, result_NodesFromWays, result_NodesWaysFromRels)
        
        if check_plot == 1:
            f, ax = plt.subplots(1)
            ax = globals()[str(item)+'_gdf_all_'+str(int(bbox[0]))+'_'+str(int(bbox[1]))].plot(ax=ax)
    ...
    OSM_features_gdf_combined.to_file(save_path +'/OSM_features_gdf_combined_'+str(int(bbox[0]))+'_'+str(int(bbox[1]))+'.shp')
...
def get_highValueArea(bbox, save_path = os.getcwd(), Low_Value_gdf = None, check_plot = 1):
    ...
    if Low_Value_gdf == None:       
        try:
            Low_Value_gdf = geopandas.read_file(save_path +'/OSM_features_gdf_combined_'+str(int(bbox[0]))+'_'+str(int(bbox[1]))+'.shp')
        except: 
            print('No Low-Value-Union found with name %s. \n Please add.' % (save_path +'/OSM_features_gdf_combined_'+str(int(bbox[0]))+'_'+str(int(bbox[1]))+'.shp') )
    else: 
    ...
    if check_plot==1:        
        ...
        plt.show()
        f.savefig('High Value Area '+str(int(bbox[0]))+'_'+str(int(bbox[1]))+'.pdf', bbox_inches='tight')
    
    return High_Value_Area

def fill_highValueArea_LitPop(bbox, country, mode,  highValueArea=None, save_path = os.getcwd(), check_plot = 1):
    """
    download values for bbox from LitPop, fill only into high-value multipolygon area
    Input:
        - bbox: List of coordinates in format [South, West, North, East]
        - Country: ISO3 code or name of country in which bbox is located
        - highValueArea: path of gdf of high-value area from previous step. If empty, searches for 
            cwd/OSM_features_gdf_combined_lat_lon.shp
        - mode: mode of re-assigning low-value points to high-value points. "nearest", "even", or "proportional"
    Returns:
        - entity (CLIMADA-compatible) with re-allocated asset values. entity_high_lat_lon
    """
    ...
    if highValueArea==None:
        try:
            High_Value_Area_gdf = geopandas.read_file(os.getcwd() + '/High_Value_Area_'+str(int(bbox[0]))+'_'+str(int(bbox[1]))+".shp")
        except: 
            print('No file found of form %s. Please add or specify path.' %(os.getcwd() + 'High_Value_Area_'+str(int(bbox[0]))+'_'+str(int(bbox[1]))+".shp"))
    else:
    ...
    # Filled Fraction of HighValue MultiPolygon
    Outer_Poly = geometry.Polygon([
                                   (bbox[1],bbox[2]),
                                   (bbox[1],bbox[0]),
                                   (bbox[3],bbox[0]),
    ...
    filled_frac = (High_Value_Area_gdf.geometry.area/Outer_Poly.area) #.area only works with geopandas_read --> multipoly format
    ...
    if mode == "nearest":       
        ...
        for i in range(0,len(ent_sub_low)):
            nearest = ent_sub_high.geometry == nearest_points(ent_sub_low.iloc[i].geometry, pts)[1] #point
        ...
        ent_sub_high.rename(columns={'value': 'value_old', 'combinedValNN': 'value'}, inplace=True)
        ...
        ent_sub_high.rename(columns={'value': 'value_old', 'combinedValeven': 'value'}, inplace=True)
        ...
        ent_sub_high.rename(columns={'value': 'value_old', 'combinedValprop': 'value'}, inplace=True)
    ...
    ent_sub_high_exp.write_hdf5(save_path + '/entity_high_'+str(int(bbox[0]))+'_'+str(int(bbox[1]))+'.h5')
    # plotting
...
def fill_highValueArea_Houses(highValueArea, mode="default", country = None, save_path = os.getcwd(), check_plot = 1):
    """
    Assign values to midpoints of individual houses according to surface area and country, gives back CLIMADA-compatible entity structure
    
    Input:
        - highValueArea: absolute path for gdf of building features queried form get_features_OSM()
        - mode: "LitPop" or "default": Default assigns a value of 5400 Chf to each m2 of building, LitPop assigns total LitPop value for the region proportionally to houses (by base area of house)
        - Country: ISO3 code or name of country in which entity is located. Only if mode = LitPop
    Returns:
        - ent_building (CLIMADA-compatible) with allocated asset values. Saved as entity_buildings_lat_lon.h5
    """
    ...
    for index, row in High_Value_Area_gdf.iterrows():
        High_Value_Area_gdf.loc[index,"Midpoint"] = High_Value_Area_gdf.loc[index,"geometry"].centroid.wkt
    ...
    High_Value_Area_gdf = High_Value_Area_gdf.rename(columns={'geometry': 'geo_polys', 'Midpoint':'geometry'})
    ...
    if mode == "LitPop":       
        ...
        for index, row in High_Value_Area_gdf.iterrows():
            High_Value_Area_gdf.loc[index, 'value'] = High_Value_Area_gdf.loc[index, 'projected_area']/totalArea*totalValue
               
    elif mode == "default": # 5400 Chf / m2 base area
        High_Value_Area_gdf['value'] = 0
        for index, row in High_Value_Area_gdf.iterrows():
            High_Value_Area_gdf.loc[index, 'value'] = High_Value_Area_gdf.loc[index, 'projected_area']*5400
    
    # put back into CLIMADA-compatible entity format and save as hdf5 file:
    ent_buildings = Exposures(High_Value_Area_gdf)
    ent_buildings.set_lat_lon()
    ent_buildings.check()
    ent_buildings.write_hdf5(save_path + '/entity_buildings_'+str(int(bbox[0]))+'_'+str(int(bbox[1]))+'.h5')
