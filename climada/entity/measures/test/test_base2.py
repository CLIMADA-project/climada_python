...
def test_measure_init(hazard, exposures, impfset):
    ...
    # Identity test: applying to a string should return the same string
    assert meas.hazard_change(hazard) is hazard
    assert meas.hazard_change(exposures) is exposures
    assert meas.hazard_change(impfset) is impfset
