...
def schedule_run(run_nb,flag,RF_model,CL_model,dis_name):
    if not flag:
 ...
 #       run_index.write("\n")
    ...
    if args.dry:
        ...
        run_params = {
            ...
            "options": "--RF_model %s --CL_model %s --dis_name %s"%(RF_model, CL_model, dis_name),
...
for rf_model in RF_MODEL:
    for cl_model in CL_MODEL:
        schedule_run(run_nb=enum,flag=single,RF_model=rf_model,CL_model=cl_model, dis_name = 'Daily')
        enum += 1
if num > 1:
